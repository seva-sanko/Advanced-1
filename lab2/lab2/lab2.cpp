...
int main()
{
	...
	//Условие if (this == &v) проверяет, не является ли объект, для которого вызывается 
	//оператор присваивания (this), тем же самым объектом, который попытаемся присвоить (&v).
	...
	// Присвоение объекта a объекту b
	b = a;

	// Вывод значений после присваивания
	cout << "\n";
	cout << "Вывод значений после присваивания:\n";
	a.Out();		b.Out();

	// В пошаговом режиме с заходом в функции (F11)
	// просмотрите ход выполнения вашей, а также следующей операции
	a = a;
	//объект равен самому себе, поэтому вместо присваивания операция возвращает ссылку на этот же объект
	stop

		// Почему работает следующий код, хотя операция присвоения не вызывается?
		// Убедитесь с помощью отладчика (точки останова).
		Vector c = a;//один из конструктороов копирования
	                 //при создании объекта мы можем инициализировать все поля класса приравняв его к уже инициализированному классу
	...
	MyString ss = s;  // нет, так как вызывается конструктор копирования
...
// Добейтесь корректного функционирования следующего фрагмента
	...
	//Vector::PrintCount(); // Вызываем от имени класса
...
// функционирования Ваших классов.
	...
	cout << "\n\n\tTest abstract Shapes\n";
	{
		...
		// Освобождение памяти и удаление объектов
		for (Shape* shape : shapes) {
			delete shape;
		}
		Shape::PrintCount();
